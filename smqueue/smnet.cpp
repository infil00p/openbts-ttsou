...
SMnet::deliver_msg_datagram(SMqueue::short_msg_pending *smp)
{
	...
	// cout << endl << "--Deliver message:" << endl;
	// cout << smp->text << endl;
	...
	if (s != 0) {
		cerr << "deliver_msg_datagram() can't lookup addr/port: " 
		     << host << ":" << port << ", error " << s << endl;
		return false;
	}
	...
	if (ap == NULL) {		// If we walked off bottom of list,
		cerr << "Couldn't send datagram to " << host << ":" 
		     << port << " on any socket" << endl;
...
SMnet::myhostname()
{
	...
	if (i != 0) {
		cerr << "myhostname() can't find our name: error "
		     << gai_strerror(i) << endl;
	}
...
SMnet::string_addr (struct sockaddr *sa, socklen_t len, bool withport)
{
	...
	if (len != 0) {
		...
		if (i != 0) {
			cerr << "string_addr() can't print IP address: error "
			     << gai_strerror(i) << endl;
			if (0 == inet_ntop(sa->sa_family, sa->sa_data, p, plen)) {
				cerr << "string_addr() can't print IP address: inet_ntop error "
				     << strerror(errno) << endl;
			}
...
SMnet::parse_addr (const char *str, char *sockad, socklen_t maxlen, socklen_t *len)
{
	...
	s = getaddrinfo(host, port, &myhints, &myaddrs);
	if (s != 0) {
		cerr << "parse_addr() can't lookup addr/port: " 
		     << host << ":" << port << ", error " << s << endl;
		return false;
	}
...
#define	RAND_DEVICE	"/dev/urandom"
	...
	if (random_fd <= 0) {
		...
		if (random_fd < 0) {
			cerr << "Can't open " << RAND_DEVICE << endl;
		}
	}
	...
	do {
		if (random_fd > 0) {
			...
			if (i != sizeof(randy.randbuf)) {
				cerr << "Can't read from " << RAND_DEVICE << endl;
