...
SMq::process_timeout()
{
	...
	while (true) {
		...
		case NO_STATE:
			...
			cerr << "== This message had an error and is being deleted:"
			     << endl << "MSG = " << qmsg->text << endl;
		...
		default: 
			cerr << "Message timed out with bad state "
...
SMq::handle_response(short_msg_p_list::iterator qmsgit)
{
	...
	if (!find_queued_msg_by_tag(sent_msg, qmsg->qtag, qmsg->qtaghash)) {
		// No message in queue.
		cerr << "Couldn't find message for response tag '" 
	...
	cerr << "Got " << qmsg->parsed->status_code
	...
	if (   sent_msg->state != ASKED_FOR_MSG_DELIVERY
		...
		cerr << "*** That's not a pleasant state. ***" << endl;
	...
	case 2:	// 2xx -- success.
		...
		if (sent_msg->parsed && 
			...
			if (!get_link(oldsms, sent_msg)) {
				cerr << "Can't find SMS message for newly "
		...
		cerr << "Deleting sent message." <<endl;
	...
	case 5: // 5xx -- failure by server (poss. congestion)
		...
		// or longer???
		cerr << "CONGESTION at OpenBTS\?\?!" << endl;
		break;
	...
	default:
		cerr << "Unknown status code in SIP response." << endl;
		break;
	}
...
//		cerr << "Param " << param->gname << "=" << param->gvalue << endl;
...
short_msg_pending::check_host_port (char *host, char *port)
{
	...
	if (!strcmp ("localhost", host) ||
		...
		if (0 == warn_once++) {
			cerr << "Accepting SIP Message from " << host <<
...
SMq::bounce_message(short_msg_pending *sent_msg, const char *errstr)
{
	...
	cerr << "Bouncing " << sent_msg->qtag << " from "
	     << sent_msg->parsed->from->url->username  // his phonenum
	     << " to " << username << ": " << errstr << endl;
...
SMq::handle_sms_message (short_msg_p_list::iterator qmsg)
{
	...
	if (0 != strcmp(qmsg->parsed->sip_method, "MESSAGE")) {
		cerr << "Invalid incoming SIP message, method is "
		     << qmsg->parsed->sip_method << endl;
		return NO_STATE;
	}
	...
	if (shortit != short_code_map.end()) {
		...
		switch (sca) {
		case SCA_REPLY:
			cerr << "Short-code replies: "
			     << params.scp_reply << endl;
			status = originate_sm(
			...
			cerr << "Reply failed " << status << "!" << endl;
			// NO BREAK
		default:
		case SCA_INTERNAL_ERROR:
			cerr << "Error in short-code function "
			     << qmsg->parsed->req_uri->username
...
SMq::lookup_uri_imsi (short_msg_pending *qmsg)
{
	...
	if (username[0] == '+' || (   0 != strncmp("imsi", username, 4)
		...
		if (!newdest) {
			...
			cerr << "Lookup phonenum '" << username << "' to IMSI failed."
			     << endl;
			 // cerr << "MSG = " << qmsg->text << endl;
		...
		char *netaddr, size_t netaddrlen)
{
	...
	response.make_text_valid();
	cerr << "Responding with \"" << errcode << " " << phrase
	     << "\"." << endl;
	
	okay = my_network.send_dgram(response.text, strlen(response.text),
				     netaddr, netaddrlen);
	if (!okay)
		cerr << "send_dgram had trouble sending the response." << endl;
}
...
#undef DEBUG_Q
#ifdef DEBUG_Q
	cerr << "=== Top of main_loop: queue:" << endl;
	debug_dump();
	cerr << "============== End of queue.  timeout = " << timeout << endl;
#else
	...
	if (len < 0) {
		// Error.
		cerr << "Error from get_next_dgram: " << strerror(errno)
		     << endl;
		// Just continue...
	} else if (len == 0) {
		...
		if (errcode == 0) {
			...
			cerr << "Received bad " << errcode
			     << " datagram:" << endl
	                     << "BADMSG = " << smp->text << endl;
		}
...
void SMq::debug_dump() {
	...
	for (; x != time_sorted_list.end(); ++x) {
		x->make_text_valid();
		cout << "== State: " << sm_state_string (x->state) << "\t"
...
SMq::save_queue_to_file(std::string qfile)
{
	...
	ofile.close();
	if (result) {
		cerr << "Saved " << howmany << " queued messages to " << qfile << endl;
	} else {
		cerr << "FAILED to save " << howmany << " queued messages to " << qfile << endl;
	}
	return result;
}
...
SMq::read_queue_from_file(std::string qfile)
{
		...
		if (errcode == 0) {
			if (MSG_IS_REQUEST(smp->parsed)) {
				...
				cerr << "Read bad SMS "
				     << smp->parsed->status_code
				     << " Response '"
				     << smp->qtag << "':" << msgtext << endl;
			}
			insert_new_message (*smpl, mystate, mytime);
		} else {
			cerr << "Read bad " << errcode
			     << " message:" << endl
	                     << "BADMSG = " << smp->text << endl;
			howmanyerrs++;
		}
	...
	cerr << "=== Read " << howmany << " messages total, " << howmanyerrs
	     << " bad ones." << endl;
	ifile.close();
	return true;
}
...
main(int argc, char **argv)
{
  ...
  //  print_as_we_validate = gConfig.getBool("Debug.print_as_we_validate");
    print_as_we_validate = 'y' == (gConfig.getStr("Debug.print_as_we_validate"))[0];
    ...
    cerr << "My own IP address is configured as " << smq.my_ipaddress << endl;
    cerr << "The HLR registry is at " << smq.my_register_hostport << endl;

    savefile = gConfig.getStr("savefile");

    if (!smq.read_queue_from_file (savefile)) {
	cerr << "Failed to read queue from file " << savefile << endl;
    }
    cerr << "Queue contains " << smq.time_sorted_list.size() 
         << " msgs." << endl;
    ...
    if (smq.reexec_smqueue) {
      cerr << "====== Re-Execing! ======" << endl;
      if (!smq.save_queue_to_file(savefile)) {
	cerr << "OUCH!  Could not save queue to file " << savefile << endl;
      }
      please_re_exec = true;
